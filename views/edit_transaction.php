<?php
// This file is loaded via MoneyController::showEditForm()
// Variables like $transaction, $categories are available from the controller
...
<main class="flex-grow">
    <section class="w-full max-w-screen-xl mx-auto px-6 md:px-8 py-10">
        ...
        <section id="EditTransactionForm">
            <div class="card max-w-3xl mx-auto bg-base-100 shadow-soft p-8 md:p-12 rounded-box">
            <form class="space-y-8" method="POST" action="money.php?action=update">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-2">
                    <div>
                    <input type="radio" name="type" id="income_radio" value="income" <?php echo ($transaction['transaction_type'] == 'income') ? 'checked' : ''; ?> class="hidden"><label for="income_radio" class="flex items-center gap-3 p-4 border-2 border-base-300 rounded-lg cursor-pointer transition-all duration-300"><svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" role="img" width="1em" height="1em" viewBox="0 0 20 20" data-icon="heroicons:arrow-up-circle-20-solid" class="iconify text-2xl"><path fill="currentColor" fill-rule="evenodd" d="M10 18a8 8 0 1 0 0-16a8 8 0 0 0 0 16m-.75-4.75a.75.75 0 0 0 1.5 0V8.66l1.95 2.1a.75.75 0 1 0 1.1-1.02l-3.25-3.5a.75.75 0 0 0-1.1 0L6.2 9.74a.75.75 0 1 0 1.1 1.02l1.95-2.1z" clip-rule="evenodd"></path></svg><span class="font-medium peer-checked:text-white">Income</span></label>
                    </div>
                    <div>
                    <input type="radio" name="type" id="expense_radio" value="expense" <?php echo ($transaction['transaction_type'] == 'expense') ? 'checked' : ''; ?> class="hidden"><label for="expense_radio" class="flex items-center gap-3 p-4 border-2 border-base-300 rounded-lg cursor-pointer transition-all duration-300"><svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" role="img" width="1em" height="1em" viewBox="0 0 20 20" data-icon="heroicons:arrow-down-circle-20-solid" class="iconify text-2xl"><path fill="currentColor" fill-rule="evenodd" d="M10 18a8 8 0 1 0 0-16a8 8 0 0 0 0 16m.75-11.25a.75.75 0 0 0-1.5 0v4.59L7.3 9.24a.75.75 0 0 0-1.1 1.02l3.25 3.5a.75.75 0 0 0 1.1 0l3.25-3.5a.75.75 0 1 0-1.1-1.02l-1.95 2.1z" clip-rule="evenodd"></path></svg><span class="font-medium peer-checked:text-white">Expense</span></label>
                    </div>
                </div>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="form-control">
                    <label for="amount" class="label"><span class="label-text text-base font-medium">Amount</span></label><label class="input input-bordered flex items-center gap-2 mt-2"><span class="text-base-content/50 font-medium">RM</span><input type="number" name="amount" id="amount" placeholder="0.00" step="0.01" min=0 required value="<?php echo htmlspecialchars($transaction['amount']); ?>" class="grow"></label>
                </div>
                <div class="form-control">
                    <label for="date" class="label"><span class="label-text text-base font-medium">Date of Transaction</span></label>
                    <input type="date" name="date" id="date" required class="input input-bordered w-full mt-2" value="<?php echo htmlspecialchars($transaction['transaction_date']); ?>">
                </div>
                </div>
                <div class="form-control">
                ...
                <select name="category" id="category" required class="select select-bordered w-full mt-2">
                    ...
                    <?php foreach ($categories as $category): ?>
                        <option value="<?php echo htmlspecialchars($category['name']); ?>" <?php echo ($transaction['category_name'] == $category['name']) ? 'selected' : ''; ?>><?php echo htmlspecialchars(ucwords(strtolower($category['name']))); ?></option>
                    <?php endforeach; ?>
                ...
                <div class="form-control md:col-span-2">
                        ...
                        <textarea name="description" placeholder="e.g., Dinner with friends" class="textarea textarea-bordered w-full"><?php echo htmlspecialchars($transaction['description']); ?></textarea>
                    </div>
                <div class="flex flex-col-reverse sm:flex-row gap-4 justify-end pt-4">
                    <button type="button" class="btn btn-ghost" onclick="window.location.href='money.php'">Cancel</button>
                    
                    <a href="money.php?action=delete&id=<?php echo htmlspecialchars($transaction['transaction_id']); ?>" 
                       class="btn btn-error" 
                    ...
                    <button type="submit" class="btn btn-primary">
                        <svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" role="img" width="20" height="20" viewBox="0 0 20 20" data-icon="heroicons:check-circle" data-width="20" class="iconify iconify--heroicons"><path fill="currentColor" fill-rule="evenodd" d="M10 18a8 8 0 1 0 0-16a8 8 0 0 0 0 16m3.707-9.293a1 1 0 0 0-1.414-1.414L9 10.586l-1.293-1.293a1 1 0 0 0-1.414 1.414l2 2a1 1 0 0 0 1.414 0l4-4z" clip-rule="evenodd"></path></svg>
                        Update Transaction
                    </button>
                </div>
            </form>
